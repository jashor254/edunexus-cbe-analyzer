...
export default function ClinicPage() {
  ...
  const loadStudents = useCallback(async () => {
    ...
    try {
      ...
      // Supplement with curriculum_type and year_level (not in API select)
      if (students.length > 0) {
        const supabase = createClient()
        const { data: extras } = await supabase
          .from('students')
          .select('id, curriculum_type, year_level')
        if (extras) {
          const map = Object.fromEntries(extras.map(e => [e.id, e]))
          students = students.map(s => ({
            ...s,
            curriculum_type: map[s.id]?.curriculum_type ?? 'cbc',
            year_level:      map[s.id]?.year_level      ?? null,
          }))
        }
      }
        ...
        {!loading && !error && students.length === 0 && (
          <div className="bg-white rounded-3xl p-12 text-center border-2 border-dashed border-slate-200 shadow-sm">
            ...
            <p className="text-slate-500 mb-8 max-w-sm mx-auto">
              Add your first student to start tracking CBC or IGCSE progress and getting personalized insights.
            </p>
            <button
              onClick={() => setShowModal(true)}
              className="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl font-bold hover:from-blue-700 hover:to-indigo-700 transition-all shadow-lg"
            >
              <PlusCircle className="w-5 h-5" />
