...
| Metric | Value |
|---|---|
| **Overall Completion** | ~62% (was ~60% after Sprint 2 — Sprint 3 fixed the root cause Sprint 2's honest downgrade was about) |
| SMS Completion | ~40% |
| Teacher Workspace / LMS Completion | ~63% |
| Parent Portal Completion | ~38% |
| Learner Portal Completion | ~67% (was ~57% — Sprint 2 downgraded this honestly when Blocker #5 was discovered; Sprint 3 fixed Blocker #5 itself, so Progress/Career Explorer/Holiday Planner/Resources/Calendar are now genuinely, not just structurally, reachable) |
| Intelligence Platform Completion | ~94% |
| Architecture Health | ~67% |
| **Pilot Readiness** | **Closer, not there yet.** Both Critical RLS holes fixed (Sprint 1). Clinic payment bypass fixed (Sprint 2). The Learner Portal's root-cause unreachability (Blocker #5) is fixed (Sprint 3) — a real self-login student account now works end-to-end, verified by 30 real signed-in HTTP tests. Remaining before pilot: land the still-uncommitted work, decide a real "sign up as a student" entry point (a product/marketing decision, not a code gap), and the items below. |
...
6. **Career Intelligence's Sprint 29/30/31 NO-GO still stands at the computation layer.** `app/api/career/capability-matches/route.ts` and `app/api/parent/career-intelligence/route.ts` both call `computeCapabilityMatches()` directly — ungated at the source; only a `shapeForGrade()` wrapper reshapes the *display*. The already-correct, grade-gated `buildCareerIntelligence()` exists and is used only by Holiday Planner. (Note: the **student-facing** Career Explorer UI itself was re-verified this pass and its grade gate now works correctly at the display layer — the gap is specifically the two routes above still bypassing the canonical function.)
7. **Portfolio and Achievements are fully dead code.** `lib/learnerPortfolio/` and `lib/learnerAchievement/` are built, tested, and referenced by zero API routes and zero pages. This is a near-zero-cost win sitting unclaimed — now unblocked, since Blocker #5 no longer stands in the way of wiring them into the (now-reachable) canonical student tree.
...
| Progress | ✅ | ✅ | ✅ | Blocker #5 resolved (Sprint 3) — reachable at /student/progress | — |
| Career Explorer | ✅ | ✅ | ✅ | Blocker #5 resolved (Sprint 3) — reachable at /student/career | — |
...
| Portfolio | ❌ | ❌ | ❌ | **Dead code — see Blocker #6** | Medium |
| Achievements | ❌ | ❌ | ❌ | **Dead code — see Blocker #6** | Medium |
| Remedials | 🟡 | 🟡 | ❌ | Teacher-only view, no student/parent surface | Medium |
| Holiday Planner | ✅ | ✅ | ✅ | Blocker #5 resolved (Sprint 3) — reachable at /student/holiday | — |
...
5. **Wire Portfolio and Achievements into the (now genuinely reachable) student UI** — both fully built and tested, zero routes calling them. No longer blocked by #5.
6. **Close the Career Intelligence grade-gating gap at the source** — point `capability-matches` and `parent/career-intelligence` routes at the already-correct `buildCareerIntelligence()` instead of the ungated `computeCapabilityMatches()`.
