...
| Metric | Value |
|---|---|
| **Overall Completion** | ~66% (was ~65% — Sprint 8 closed out the Review Dashboard and fixed a real publish-safety gap) |
| SMS Completion | ~40% |
| Teacher Workspace / LMS Completion | ~66% (was ~65% — Sprint 8's Review Dashboard rebuild (batched fetch, filter/search, bulk approve/reject, Generate All) moved both Variant Generation and Review Dashboard rows to ✅✅✅; Academic Clinic downgraded to 🟡🟡🟡 on the newly-confirmed finding that it never reads adaptive-quiz scores) |
...
9. ~~**Publishing an adaptive assignment had zero variant-approval validation.**~~ **RESOLVED 2026-07-20 (Sprint 8).** `PATCH /api/teacher/assignments/[id]` would flip status to `active` regardless of approved-variant coverage — a question with no approved variant silently falls back to the canonical question for every student, defeating "adaptive" with no visible sign anything was skipped, on a page whose own copy implied a check that never existed. Fixed with a named, computed warning (unapproved-question count) plus a required confirmation checkbox before publish — not a hard block, since a teacher may have a legitimate reason to publish partially adaptive.
10. **Academic Clinic and Report Cards never read adaptive-quiz scores.** Found Sprint 8: both `lib/career/clinicReportBuilder.ts`/`autoReportGenerator.ts` (Clinic) and `lib/core/report-cards.ts` (Report Cards) read only from the legacy `assessments` table / Core schema's own marks repos — zero reference to `learner_evidence`, `assignment_submissions`, or Projection in either. An adaptive-quiz score is confirmed reaching Gradebook, Blueprint, Evidence, Projection, Parent Portal, and Learner Timeline correctly, but never Clinic or Report Cards. Not fixed — this means changing what two mature, independently-designed report pipelines read from, squarely inside "do not redesign the assessment domain." The single largest remaining Assessment Excellence gap; needs its own properly-scoped sprint.
11. **`learner_evidence`'s claim key can silently supersede a second same-term adaptive quiz.** Found Sprint 8: the claim key (`lib/intelligence/evidenceLifecycle.ts`) is `learnerId:subject:assessmentType:academicYear:term` — no `evidenceSource`/`assignmentId` component, despite the type signature listing `evidenceSource`. A second adaptive quiz (or any teacher-marked assignment) in the same subject/term silently supersedes the first quiz's evidence instead of coexisting. Not fixed — the claim key is cross-cutting (every evidence type, not just quizzes), out of this sprint's blast radius; needs careful, scoped analysis of its own.
12. **No audit trail for who approved/rejected a variant.** Found Sprint 8: `assignment_question_variants` has no `approved_by`/`rejected_by` actor column and neither `approveVariant()`/`rejectVariant()` calls an audit-event insert — contrast `learner_evidence`'s own lifecycle, which does call `insertAuditEvents`. A real, additive, low-risk fix (two nullable columns + two small function updates) but requires a migration not pre-approved for Sprint 8 — recommended as the natural next small sprint.
...
| Adaptive Assignments — Variant Generation | ✅ | ✅ | ✅ | Sprint 8: "Generate All" batches per-question generation server-side (still sequential real AI calls, not parallel-flooded, and still no live progress UI for a long batch) — the "10 clicks for 10 questions" friction is gone, a genuine progress indicator remains a small future polish item | Low |
| Adaptive Assignments — Review Dashboard | ✅ | ✅ | ✅ | Sprint 8: batched single-request variant fetch (was N requests for N questions), search + status + tier filters, checkbox multi-select with bulk approve/reject. `editVariant()` still has no UI — a real editing surface, correctly scoped out as a feature build rather than a dashboard-efficiency fix | Low |
...
| Academic Clinic | ✅ | 🟡 | 🟡 | **Confirmed Sprint 8:** reads only the legacy `assessments` table (`lib/career/clinicReportBuilder.ts`, `autoReportGenerator.ts`) — an adaptive-quiz score (confirmed reaching Gradebook/Blueprint/Evidence/Projection/Parent Portal/Timeline) never reaches a Clinic report. Real gap, not attempted — fixing it means changing what a mature, independently-designed pipeline reads from, out of "do not redesign the assessment domain" | Medium |
| Risk Detection | ✅ | ✅ | ✅ | — | Low |
