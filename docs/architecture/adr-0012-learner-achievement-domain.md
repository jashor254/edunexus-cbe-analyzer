...
**Status: IMPLEMENTED (2026-07-18, Sprint 12W).** The canonical Achievement domain (schema, repository, service, Blueprint integration, tests) now exists exactly as frozen here — `learner_achievements`/`achievement_media`/`achievement_tags`/`achievement_verification_history`, the frozen six-state lifecycle (Draft/Verified/Published/Rejected/Revoked/Archived, no separate "Submitted" state — this ADR's own Phase 4 never named one), the Phase 2/Phase 8 Achievement-Type/Category split (kept as two separate fields, never merged), Phase 5's mandatory Evidence-or-verifying-document rule, and Phase 6/7's summary-only Blueprint composition. See `sprint-12w-achievement-foundation.md` for the full implementation record. No UI, uploads, certificates, badges, or Career integration exist yet — those remain future, separately-approved sprints per this ADR's own Stop Condition and Sprint 12W's own Forbidden list.

**Precedes**: Sprint 12W (the first Achievement implementation sprint, complete — see above), and every future Portfolio/Blueprint/Graduation-Profile/University-Profile/Employer-Profile sprint that needs achievement data.
