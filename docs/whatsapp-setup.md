...
### Template 3: `edunexus_school_outreach` (Growth Engine cold outreach)

**Category:** `MARKETING` — this is business-initiated contact to a school
that has never messaged EduNexus first, so it does not qualify as `UTILITY`.
Meta reviews marketing templates more strictly and can reject them; expect
slower approval than the two templates above, and be ready to revise wording
if rejected.
...
| Variable | Value |
|----------|-------|
| `{{1}}` | The full personalized message body already generated by lib/growth/messaging/generate.ts (cold intro, follow-up, demo reminder, etc.) |

One body variable carries the entire message — the same pattern as
`edunexus_parent_update` in `lib/whatsapp/sender.ts`. This means every
message variant the Growth Engine's templates.ts can produce reuses this
one Meta-approved template; a new sales message copy never needs its own
Meta submission.

**Sample message:**
```
Hi Mr. Kamau, this is Dennis from EduNexus. We help CBC schools like
Kirinyaga Girls save teachers hours of marking time with AI-assisted
grading. Would you be open to a 15-minute call this week?
```

Used by: `app/api/growth/schools/[id]/messages/send/route.ts` via
`lib/growth/messaging/send.ts`, fired only when the founder clicks
"Approve & Send" in the Communication Workspace — never automatically.
...
2. Select category: `Utility` (or `Marketing` for `edunexus_school_outreach`)
