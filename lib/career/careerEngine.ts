...
export async function getCareerBySlug(slug: string): Promise<Career | null> {
  ...
  const { data, error } = await supabase
    .from('careers')
    .select(
      'id, slug, title, category, description, ai_impact, ai_impact_level, kenya_market_outlook, salary_range_kes, pathways, required_subjects, skill_timeline, future_skills, obsolete_skills, kenya_examples, created_at, updated_at'
    )
    .eq('slug', slug)
    .single()
...
export async function searchCareers(filters: CareerSearchFilters): Promise<CareerSummary[]> {
  ...
  let query = supabase
    .from('careers')
    .select('id, slug, title, category, description, ai_impact_level, kenya_market_outlook, salary_range_kes, required_subjects')
  ...
  if (filters.ai_impact_level) {
    query = query.eq('ai_impact_level', filters.ai_impact_level)
  }
...
export async function getCareersByCategory(category: string): Promise<CareerSummary[]> {
  return searchCareers({ category: category as CareerSearchFilters['category'] })
}
...
export function getNextSkillsForAge(timeline: SkillTimelineItem[], age: number): SkillTimelineItem | null {
  ...
  const currentIdx = ranges.indexOf(currentLabel)
  if (currentIdx === -1 || currentIdx >= ranges.length - 1) return null
  const nextLabel = ranges[currentIdx + 1]
  return timeline.find((t) => t.age_range === nextLabel) ?? null
}

// ──────────────────────────────────────────────────────────
// STUDENT MATCH OPERATIONS
// ──────────────────────────────────────────────────────────
...
export async function getMatchesForStudent(studentId: string): Promise<CareerMatchWithDetail[]> {
  ...
  const { data, error } = await supabase
    ...
    .select(
      `id, student_id, career_id, match_score, match_reasoning, subject_gaps, skill_gaps, generated_at,
       career:careers(id, slug, title, category, description, ai_impact_level, kenya_market_outlook, salary_range_kes, required_subjects)`
    )
  ...
  return (data ?? []) as unknown as CareerMatchWithDetail[]
}
...
export async function generateParentSummary(studentId: string): Promise<ParentCareerSummary> {
  ...
  const overallReadiness = topCareers.length > 0
    ? `${student.name} shows strongest alignment with ${topCareers[0]?.career.title ?? 'multiple careers'} based on their current subject performance.`
...
export async function getCareersBySubjectStrength(
  ...
  if (error) throw new Error(`Failed to load careers: ${error.message}`)
  const all = (data ?? []) as CareerSummary[]
  ...
  return scored.map((s) => s.career)
}

// ──────────────────────────────────────────────────────────
// SEED RUNNER (called from admin API)
// ──────────────────────────────────────────────────────────
