// lib/sow/schemeLessonsSubStrandId.integration.test.ts
//
// H5A-2 — proves scheme_lessons.sub_strand_id actually does what it exists
// to do: persist the exact canonical sow_substrands.id selected upstream
// (CUR-SOW-001), reject a fabricated id via a real FK constraint (mirroring
// lib/curriculum/assignmentSubstrandId.integration.test.ts's proof for
// assignments.substrand_id), stay NULL when no canonical identity was ever
// available rather than have one inferred from text (CUR-SOW-002), and
// never confuse two sub-strands that happen to share a title
// (CUR-ID-003/CUR-SOW-001 collision case). Exercises the exact insert shape
// app/api/sow/save/route.ts uses (verified against that file) against a
// real synthetic schemes_of_work/scheme_lessons fixture — the real
// production writer, not a reimplementation of its logic.
//
// Run: npx tsx --env-file=.env.local --test lib/sow/schemeLessonsSubStrandId.integration.test.ts
import { test, before, after } from 'node:test'
import assert from 'node:assert/strict'
import { createTestServiceClient as createServiceClient } from '@/utils/supabase/test-service'

const SYNTHETIC_MARKER = 'SYNTHETIC_H5A2_SCHEME_LESSONS_TEST'
...
before(async () => {
  ...
  const { data: auth, error: authErr } = await db.auth.admin.createUser({
    email: `h5a2-scheme-lessons-${Date.now()}@example.com`,
...
after(async () => {
  await db.from('scheme_lessons').delete().eq('scheme_id', schemeId)
...
// The exact insert shape app/api/sow/save/route.ts uses for scheme_lessons,
// minus the fields irrelevant to this contract.
function lessonRow(week: number, lesson: number, strand: string, substrand: string, subStrandId: string | null) {
  return {
    scheme_id: schemeId, week, lesson, strand, substrand, sub_strand_id: subStrandId,
    learning_outcomes: '[]', learning_experiences: '[]', key_inquiry_questions: '[]',
    learning_resources: '[]', assessment_methods: '[]',
  }
}

test('CUR-SOW-001: a real sow_substrands.id persists exactly on scheme_lessons.sub_strand_id', async () => {
  const { data, error } = await db
    .from('scheme_lessons')
    .insert(lessonRow(1, 1, SYNTHETIC_MARKER, 'Fractions (synthetic)', subStrandXId))
...
test('CUR-SOW-002: no canonical id upstream persists as NULL, text preserved, never fabricated', async () => {
  const { data, error } = await db
    .from('scheme_lessons')
    .insert(lessonRow(1, 2, SYNTHETIC_MARKER, 'Some AI-generated topic text', null))
    .select('id, sub_strand_id, strand, substrand')
    .single()

  assert.equal(error, null)
  assert.equal(data!.sub_strand_id, null)
  assert.equal(data!.substrand, 'Some AI-generated topic text')
})

test('scheme_lessons.sub_strand_id rejects a fabricated id — the FK constraint is real, not a UI convention', async () => {
  const fakeId = '00000000-0000-0000-0000-000000000000'
  const { error } = await db
    .from('scheme_lessons')
    .insert(lessonRow(1, 3, SYNTHETIC_MARKER, 'Fractions (synthetic)', fakeId))
...
test('pre-existing scheme_lessons rows (no sub_strand_id at all) remain valid — no backfill, no rewrite', async () => {
  const row = lessonRow(1, 4, SYNTHETIC_MARKER, 'Legacy-shaped text', null)
  delete (row as { sub_strand_id?: string | null }).sub_strand_id
  const { data, error } = await db
    .from('scheme_lessons')
    .insert(row)
    .select('id, sub_strand_id, substrand')
    .single()

  assert.equal(error, null)
  assert.equal(data!.sub_strand_id, null)
  assert.equal(data!.substrand, 'Legacy-shaped text')
})

test('CUR-ID-003/CUR-SOW-001 collision: two sub-strands sharing an identical title persist to their own distinct rows, never confused', async () => {
  const { data: rowX, error: errX } = await db
    .from('scheme_lessons')
    .insert(lessonRow(2, 1, SYNTHETIC_MARKER, 'Fractions (synthetic)', subStrandXId))
    .select('id, sub_strand_id, substrand')
    .single()
  const { data: rowY, error: errY } = await db
    .from('scheme_lessons')
    .insert(lessonRow(2, 2, `${SYNTHETIC_MARKER}_COLLISION`, 'Fractions (synthetic)', subStrandYId))
    .select('id, sub_strand_id, substrand')
    .single()

  assert.equal(errX, null)
  assert.equal(errY, null)
  assert.equal(rowX!.substrand, rowY!.substrand, 'both share the same title — the interesting case')
