// lib/learnerIntelligence/careerIntelligenceOrchestration.ts
// Career Intelligence — DB/Projection/AI-backed orchestration.
//
// Sibling to lib/learnerIntelligence/careerIntelligence.ts (pure reasoning).
// Every Career operation that reads a student's profile, recomputes
// Projection, or looks up career data lives here — never in the pure file.
// Imports pure reasoning FROM careerIntelligence.ts; the pure file must
// never import anything from this one. See careerIntelligence.ts's own
// header comment for the full rationale (Pure-Domain Test Isolation Audit /
// Career Intelligence Canonical Boundary Purity Audit).
//
// Moved verbatim from careerIntelligence.ts — no logic changes, no output-
// shape changes, only import/location relocation.
...
import type { CareerDoor, DoorType, CareerMatchWithDetail, CareerSummary, CapabilityProfile } from '@/lib/career/types'
...
//
// `lib/career/careerEngine.ts`'s `getCapabilityProfile()` reads a PERSISTED
// snapshot (`students.capability_profile`) written by
// `recomputeAndSaveCapabilityProfile()` — which deliberately blends this
// same canonical Projection data with the legacy `assessments` table
// (Decision 8 amended, that file's own header comment), kept specifically
// so a student whose only capability signal predates Evidence Domain
// coverage — or arrived through `app/api/assessments/create/route.ts`,
// confirmed to have zero live UI caller today, i.e. effectively
// unreachable — is not silently shown "no data." That blend is legitimate
// coverage insurance, not something this phase deletes.
//
// What WAS a bug (Phase 3's finding, reproduced): `/api/career/capability`
// (reads this persisted snapshot only) and `/api/career/capability-matches`
// (reads `resolveFreshCapabilityProfile` live) are shown on the SAME
// Career Explorer page and could disagree for the same learner at the
// same moment, because one always prefers a possibly-stale write and the
// other always prefers live truth. This function is the fix: canonical
// Projection FIRST, the persisted blend only on genuine ABSENCE of
// canonical evidence — the identical "never compared, legacy only on
// absence" precedent already established for Compass
// (resolveCompassAcademicLevel) and reused here, not a new pattern.
export async function resolveCurrentCapabilityProfile(studentId: string): Promise<CapabilityProfile | null> {
  const fresh = await resolveFreshCapabilityProfile(studentId)
  if (fresh) return fresh.profile
  return getCapabilityProfile(studentId)
}
