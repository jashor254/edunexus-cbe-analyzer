...
test('flags a low-confidence career narrative with no hedging language', () => {
  const bp = baseBlueprint({
    career: available<CareerData>({ careerCluster: 'Agriculture', strengthProfile: 'You are a strong match for this career.', futureDirection: 'Pursue this path directly.', aiOutlook: null, confidence: 'Low', doorsPreview: null, aiChangeSummary: null, humanAdvantageSummary: null, explorationSuggestions: null, notes: [] }),
  })
  const report = validateBlueprintCoherence(bp, [])
...
test('flags a career claim with no confidence label at all (confidence mismatch)', () => {
  const bp = baseBlueprint({
    career: available<CareerData>({ careerCluster: 'Agriculture', strengthProfile: 'A strong fit.', futureDirection: null, aiOutlook: null, confidence: null, doorsPreview: null, aiChangeSummary: null, humanAdvantageSummary: null, explorationSuggestions: null, notes: [] }),
  })
  const report = validateBlueprintCoherence(bp, [])
