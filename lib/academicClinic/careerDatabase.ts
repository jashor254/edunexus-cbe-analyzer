...
// HAKIKISHA NENO 'export const' LIPO HAPA
...
// HAKIKISHA NENO 'export function' LIPO HAPA
export function getMatchingCareers(pathway: string, scores: Record<string, number>): CareerData[] {
  if (!CAREER_DATABASE) return [];
  // Filter by pathway and return top matches
  return CAREER_DATABASE.filter(c => c.pathway === pathway).slice(0, 5);
}